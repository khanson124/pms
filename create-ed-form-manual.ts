...
async function createEDFormForEvaluation(evaluationId: number) {
    try {
        ...
        const totalAmount = evaluation.request.totalEstimated ? parseFloat(String(evaluation.request.totalEstimated)) : 0;

        let procurementTypes: string[] = [];
        if (evaluation.request.procurementType) {
            if (Array.isArray(evaluation.request.procurementType)) {
                procurementTypes = evaluation.request.procurementType.map((t: string) => t.toUpperCase());
        ...
        if (evaluation.sectionE) {
            try {
                const sectionE = typeof evaluation.sectionE === 'string' ? JSON.parse(evaluation.sectionE) : evaluation.sectionE;
        ...
        if (evaluation.sectionD) {
            try {
                const sectionD = typeof evaluation.sectionD === 'string' ? JSON.parse(evaluation.sectionD) : evaluation.sectionD;
                if (sectionD.summary) {
        ...
        const procurementManagers = await prisma.user.findMany({
            where: {
                OR: [{ role: 'PROCUREMENT_MANAGER' }, { role: 'EXECUTIVE_DIRECTOR' }],
            },
        });
